...
if [ ! -d ${CLONED_DOC} ]; then
    ...
    git clone git@github.com:hidimstat/hidimstat.github.io.git ${CLONED_DOC} --depth=1
fi
...
if [ "${DEPLOY_TYPE}" == "stable" ]; then
    ...
    MSG="https://github.com/hidimstat/hidimstat/commit/${COMMIT_SHA} : ${HEAD_COMMIT_MESSAGE}"
fi
