...
# Skip examples
if [[ "$build_type" =~ ^SKIP || "$build_type" =~ ^QUICK || "$build_type" =~ ^'BUILD: detected examples']]
then
    exit 0
fi
...
if [[ "$CIRCLE_BRANCH" =~ ^main$|^[0-9]+\.[0-9]+\.X$ ]]
then
    url="$CIRCLE_BRANCH"
else
    url="main"
...
set -o pipefail && cd tools/examples/circleci && pytest --mpl --mpl-generate-summary=basic-html --mpl-results-path=./result_test
cd -
