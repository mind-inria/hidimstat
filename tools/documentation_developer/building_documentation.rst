...
This will run all the examples, which takes a while.
You can also run only a few examples based on their file names.
...
In the case of a PR, you can look at the version generated by circle ci,
by looking at the details of `Check the rendered docs here!`.

.. note::

    Your local build of the doc and the examples may look slightly different
    than the one generated in CI
    because of potential differences in python and dependencies versions.
    To get doc build comparable to the one built in CI,
    you need to use `tox <https://tox.wiki/>`_.
