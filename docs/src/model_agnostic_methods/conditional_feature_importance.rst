...
:math:`X^{\pi(j| -j)} \perp\!\!\!\!\perp Y \mid X^{-j}`. The predictive model is then evaluated on the
modified feature vector :math:`X^{\pi(j| -j)}`, which is identical to the original feature
...
:math:`X^{\pi(j| -j)}`, and specifically to sample the :math:`j^{th}` feature independently from the conditional
distribution, :math:`[X^{\pi(j| -j)}]_j \overset{\text{i.i.d.}}{\sim} P(X^j | X^{-j})`, while breaking the
...
Then the perturbed feature :math:`[X^{\pi(j| -j)}]_j` is generated by keeping the predictable part
...
.. note:: **Estimation of** :math:`\nu_j`
    ...
    To generate the perturbed input :math:`X^{\pi(j| -j)}`, a model for :math:`\nu_j` is required.
...
* Vanishing variance: Under the null hypothesis, even if the loss difference
  ...
  using other nonparametric test or using a conditional randomization test (:ref:`sphx_glr_generated_gallery_examples_plot_holdout_randomization_test.py`).
