...
class TestTtest_1samp:
    ...
    def test_vs_nonmasked(self):
        ...
        res1 = ttest_1samp_corrected_NB(
            outcome[:, 0], 1, test_frac=1 - 1 / outcome[:, 0].shape[0]
        )
        res2 = mstats.ttest_1samp(outcome[:, 0], 1)
        assert_allclose(res1, res2)
        res3 = ttest_1samp_corrected_NB(outcome[:, 0], 1)
    ...
    def test_result_attributes(self):
        ...
        res = ttest_1samp_corrected_NB(outcome[:, 0], 1)
        attributes = ("statistic", "pvalue")
        check_named_results(res, attributes, ma=True)

    def test_empty(self):
        res1 = ttest_1samp_corrected_NB([], 1)
        assert_(np.all(np.isnan(res1)))

    def test_zero_division(self):
        t, p = ttest_1samp_corrected_NB([0, 0, 0], 1)
        ...
        with warnings.catch_warnings():
            ...
            t, p = ttest_1samp_corrected_NB([0, 0, 0], 0)
            assert_(np.isnan(t))
    ...
    def test_alternative(self, alternative):
        ...
        t_ex, p_ex = ttest_1samp_corrected_NB(
            x, 9, alternative=alternative, test_frac=1 - 1 / x.shape[0]
        )
        t, p = mstats.ttest_1samp(x, 9, alternative=alternative)
        assert_allclose(t, t_ex, rtol=1e-14)
        assert_allclose(p, p_ex, rtol=1e-14)

        t_ex_1, p_ex_1 = ttest_1samp_corrected_NB(x, 9, alternative=alternative)
