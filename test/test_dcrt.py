...
def test_dcrt_lasso():
    ...
    vi_no_screening = dcrt_pvalue(results_no_screening[0], results_no_screening[1], results_no_screening[2], results_no_screening[3], selection_only=False)
    results_screening = dcrt_zero(
        X, y, screening=True, statistic="residual", random_state=2024
    )
    vi_screening = dcrt_pvalue(results_screening[0], results_screening[1], results_screening[2], results_screening[3], selection_only=False)
    ...
    vi_no_verbose = dcrt_pvalue(results_screening[0], results_screening[1], results_screening[2], results_screening[3], selection_only=True)
    assert len(vi_no_verbose) <= 10
    
    # Checking with scaled statistics
    vi_scaled = dcrt_pvalue(results_no_screening[0], results_no_screening[1], results_no_screening[2], results_no_screening[3], selection_only=False)
    ...
    vi = dcrt_pvalue(results[0], results[1], results[2], results[3], selection_only=False)
    assert len(vi[0]) <= 10
    assert len(vi[1]) == 10
    assert len(vi[2]) == 10
    ...
    vi_refit = dcrt_pvalue(results_refit[0], results_refit[1], results_refit[2], results_refit[3], selection_only=False)
    ...
    vi_use_cv = dcrt_pvalue(results_use_cv[0], results_use_cv[1], results_use_cv[2], results_use_cv[3], selection_only=False)
    ...
    vi_covariance = dcrt_pvalue(results_covariance[0], results_covariance[1], results_covariance[2], results_covariance[3], selection_only=False)
