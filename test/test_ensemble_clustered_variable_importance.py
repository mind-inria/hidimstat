...
from hidimstat import CluVI, DesparsifiedLasso, EnCluVI
...
def test_cluvi_spatial():
    ...
    for seed in range(10):
        ...
        cluvi = CluVI(
            vi_estimator=DesparsifiedLasso(estimator=estimator),
...
def test_cluvi_independence():
    ...
    vi_estimator = DesparsifiedLasso(
        estimator=LassoCV(
            max_iter=1000, tol=0.0001, eps=0.01, fit_intercept=False
        )
    )
    ...
    c2 = CluVI(
        vi_estimator=vi_estimator, clustering=ward, cluster_bootstrap_size=0.5
    )
...
def test_encluvi_independence():
    ...
    encluvi = EnCluVI(
        vi_estimator=DesparsifiedLasso(estimator=LassoCV()),
        clustering=ward,
        n_bootstraps=20,
