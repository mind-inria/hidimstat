...
selection_vi = vi.selection(
    percentage=0.1, threshold=None, k_best=None, threshold_pvalue=None
)  # binary vector of n_features
###############################################################################
...
selection_cvi = cvi.selection(
    percentage=None, threshold=2.0, k_best=None, threshold_pvalue=None
)  # binary vector of n_features
###############################################################################
# Variable Selection:
# ------------------------------
...
selection_vs = vs.selection(
    percentage=None, threshold=None, k_best=20.0, threshold_pvalue=None
)  # binary vector of n_features
# ----------------------------
...
importance_vs, pvalue_vs = vs.importance(X, y, None)  # vector of n_features
selection_vs = vs.selection(
    percentage=None, threshold=None, k_best=20.0, threshold_pvalue=0.8
)  # binary vector of n_features
...
importances_vi = vi.importance(None, None)  # vector of n_features
importances_vi, NONE = vi.importance_fit(X, y, cv)  # vector of n_features, not p-value
selection_vi = selection(
    importances_vi, None, percentage=0.1
)  # binary vector of n_features
###############################################################################
# Controlled Variable Importance:
# -------------------------------
...
importances_cvi = cvi.importance(X[test], y[test])  # vector of n_features
importances_cvi, pvalue_cvi = cvi.importance(X, y, cv)  # vector of n_features
selection_cvi = selection(
    importances_cvi, pvalue_cvi, threshold=2.0
)  # binary vector of n_features
###############################################################################
...
selection_ivs = selection(
    importance_ivs, pvalue_ivs, k_best=20.0
)  # binary vector of n_features
###############################################################################
...
selection_mvs = selection(
    importance_mvs, pvalue_mvs, k_best=20.0, threshold_pvalue=0.8
)  # binary vector of n_features
...
selection_mvs, _ = mvs.selection(
    None, None, FDP=0.1
)  # ??2 output?? binary vector of n_features
