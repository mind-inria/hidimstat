...
# Usage the methods
# -----------------
# see the API for more details about the optional parameter: :py:func:`hidimstat.ada_svr`
...
#############################################################################
#
# The pvalue and the corrected pvalue can help to be confident in the previous results.
...
#############################################################################
# The pvalue and the corrected pvalue show that the confidence in the variable of importance
# is high. The pvalue and the corrected pvalue are close to one.
...
#############################################################################
# The results for the alternative hipothese shows that the confidence for not important variables
# is high. The 1-pvalues are not significant. However, the corrected 1-pvalue is close to one.
# We can conclude that the not importance variables are estimated in this case.

#############################################################################
#
# Principle of the methods
# ------------------------
# The ADA-SVR method is a statistical inference procedure that estimates the variable of importance of a Support Vector Regression (SVR).
# The method is a simplification of the permutation test for SVR (see :py:func:`hidimstat.permutation_test`).
# The principle is to shuffle the target variable and to estimate the variable of importance with the SVR in order to estimate the distribution of the coefficients of the SVR. For ADA-SVR, the distribution of the coefficients of the SVR is assumed to be a normal distribution centred around zeros. ADA-SVR uses the central limit theorem to estimate the 
# standard deviation of this normal distribution for each coefficient (see the following figure).
#
...
# ------------------------------------------------------------------------
#
estimator = SVR(kernel="linear", epsilon=0., gamma="scale", C=1.0)
estimator.fit(X, y)
...
#############################################################################
# **Compare the distribution of the coefficients of SVR with the estimation of the distribution 
# of the coefficients by ADA-SVR**
print("ADA-SVR assumes that the normal distribution of the coefficient is center around zeros.\n",
      "Our estimation is that the maximum deviation is: {:.4f}\n".format(np.max(np.abs(np.mean(proba, axis=0)))),
      "ADA-SVR provides that the standard deviation the normal distribution for each coefficients.\n",
      "The maximum of difference between AdA-SVR estimation and our estimation is:{:.4f}".format(
      np.max(np.abs(scale - np.std(proba, axis=0))/scale)))
plt.show()
