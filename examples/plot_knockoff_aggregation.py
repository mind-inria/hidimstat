...
def different_result(n_subjects):
    # Number of variables
    n_clusters = 100
    ...
    def plot_results(bounds, fdr, nsubjects, n_clusters, rho, power=False):
        ...
        if power:
            ...
            plt.hlines(
                fdr, xmin=0.5, xmax=3.5, label="Requested FDR control", color="red"
            )
            plt.xticks(
                [1, 2, 3],
                ["MX Knockoffs", "Quantile aggregation", "e-values aggregation"],
...
different_result(n_subjects=100)
different_result(n_subjects=50)
