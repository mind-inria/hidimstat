...
def _bin_indices(x, quantiles):
    ...
    idx = np.digitize(x, quantiles, right=False) - 1
    # samples equal to the last edge fall in the last bin
    return np.clip(idx, 0, len(quantiles) - 2)


def compute_ale_1d(estimator, X, feature_idx, grid_resolution=50, is_categorical=False):
    ...
    is_categorical : bool, default=False
        Reserved for future categorical support.  Must be ``False``.
    ...
    """
    if is_categorical:
        raise NotImplementedError("Categorical features are not yet supported. Set is_categorical=False.")
    ...
    uncentered_ale = np.array([0, *np.cumsum(mean_effects)])

    # Center: subtract the sample-weighted mean
    ale_centers = (uncentered_ale[1:] + uncentered_ale[:-1]) / 2
    uncentered_mean = np.sum(ale_centers * bin_counts) / bin_counts.sum()
    ale = uncentered_ale - uncentered_mean


    return {
        "ale": ale,
        "uncentered_ale": uncentered_ale,
        "uncentered_mean": uncentered_mean,
        "quantiles": quantiles,
    }


def compute_ale_2d(estimator, X, feature_indices, grid_resolution=20, is_categorical=(False, False)):
    ...
    result : dict with the following keys:
        ...
        ``"ale"`` : ndarray of shape (n_quantiles_i, n_quantiles_j)
            Centred 2D ALE surface evaluated at each 2D bin centre.
    ...
    # Remove main effects of both features
    #bin_counts_i = bin_counts.sum(axis=1, keepdims=True)
    #bin_counts_j = bin_counts.sum(axis=0, keepdims=True)

    #main_effects_i = np.zeros_like(mean_effects)
    #main_effects_j = np.zeros_like(mean_effects)

    #non_zero_i = bin_counts_i.ravel() > 0
    #non_zero_j = bin_counts_j.ravel() > 0

    #main_effects_i[non_zero_i, :] = np.sum(mean_effects[non_zero_i, :] * bin_counts[non_zero_i, :], axis=1, keepdims=True) / bin_counts_i[non_zero_i, :]
    #main_effects_j[:, non_zero_j] = np.sum(mean_effects[:, non_zero_j] * bin_counts[:, non_zero_j], axis=0, keepdims=True) / bin_counts_j[:, non_zero_j]

    # Double cumulative sum : uncentred 2D ALE at bin edges
